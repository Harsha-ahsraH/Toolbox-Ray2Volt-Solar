...
**Sign-in**:
Entering an Account's password once for the whole toolbox, which then hides every tool above that Account's Access Level for 12 hours and shows its name in the sidebar without an avatar badge. Everyone signs in with a blank password. **The repository is public**, so a Sign-in decides what a person is shown; it does not stop anyone fetching a tool's files directly by URL — see **Toolbox-hosted resource**.
