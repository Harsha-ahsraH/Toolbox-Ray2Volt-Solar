...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!Docs) {
        console.error('Ray2VoltFinancialDocuments module is required before proforma-invoice.js');
        return;
    }
    ...
    if (piGenerateBtn) {
        piGenerateBtn.addEventListener('click', () => {
            ...
            const piNo = piNumberInputVal || Docs.generateDocumentNumber('R2VPI');
    ...
    const piDownloadBtn = byId('piDownloadBtn');
    if (piDownloadBtn) {
        piDownloadBtn.addEventListener('click', () => {
            if (!piPreview?.classList.contains('visible')) {
                piGenerateBtn?.click();
            }

            window.Ray2VoltPdfDownload?.downloadElement({
                element: piPreview,
                button: piDownloadBtn,
                filename: `Ray2Volt-Proforma-Invoice-${byId('piDispInvoiceNo')?.textContent || 'Draft'}`
            });
        });
    }
});
